...
| | |
|---|---|
| **92 ms** | p50 for the query the product actually runs — a **3-hop undirected** neighbourhood at 100k edges, measured through `Store` on the shipped implementation ([`MEASURED.md`](MEASURED.md)). The *directed* traversal is 0.37 ms p50 at a **million** edges, and both are real; they are simply different questions. §3.1 explains which is which and why the gap is two orders of magnitude. Either way, no extension is needed to get there. |
...
|----|-----------|---------|-----------|
| **H1** | 3-hop over 100k edges p95 < 50 ms with plain tables | ⚠️ **Confirmed in design, refuted in the product** | design: directed p95 **0.579 ms**, undirected-from-hub **25.3 ms**. Shipped code, undirected: **p95 156 ms** — the threshold does not survive, the conclusion does (§3.2) |
| **H2** | A hub is > 10x slower than a random entity | ⚠️ **True at depth 2, false at depth 3** | design: undirected hub/random **3.2x**. Shipped code: **12x at 2 hops** (104.6 / 8.7 ms), **3.2x at 3** (290.6 / 92.2). Averages lie most at *shallow* depth — by three hops an ordinary entity has also reached most of the graph. Also see §8: the generator cannot build a true out-hub |
| **H3** | 4 hops returns > 20% of the graph, so depth is a product decision | ✅ **Confirmed twice, and earlier than expected** | **75%** of all entities at 100k (9,388 rows). On the shipped code *every 3-hop query* saturated its 500-row cap |
...
| **H7** | Hybrid FTS5 + graph p95 < 100 ms at 100k | ⚠️ **Confirmed in design, marginal in the product** | design p50 **26.9 ms** / p95 **33.5 ms**, 1.3x the FTS5 query alone. Shipped code: p50 53 ms, **p95 106 ms** — just outside. At 1M, FTS5 is the cost and the graph hop is free |
...
## 3. Two corrections from the shipped implementation

Everything in §2 and §5–§8 measures the *design*, in Python. A teammate then
built the migration for real and measured it through `Store`
([`MEASURED.md`](MEASURED.md), `core/examples/graph_bench.rs`). It found two
things this benchmark could not, and both change what should be published.

### 3.1 `JOIN` is a landmine. Use `CROSS JOIN`.

`MEASURED.md` found the k-hop query as originally written here — `FROM reach x
JOIN relations r ON …` — running **903 ms** where `CROSS JOIN` ran **14 ms**,
a 64x gap on identical schema and data. This benchmark never saw it. The
reason is worth more than either number, so it was tested directly
([`bench/join_vs_crossjoin.py`](bench/join_vs_crossjoin.py),
[`out/join-vs-crossjoin.json`](out/join-vs-crossjoin.json)) — 2-hop undirected,
100k edges, same file, four cells:

| | with `ANALYZE` | **without `ANALYZE`** |
|---|---:|---:|
| `JOIN` | 0.306 ms | **1,075.665 ms** |
| `CROSS JOIN` | 0.243 ms | 0.320 ms |

**3,515x**, and only in the bottom-left cell. The mechanism is visible in
`EXPLAIN QUERY PLAN`: with no `sqlite_stat1`, the planner makes `relations` the
**outer** loop matched on `scope=?` alone — which selects three quarters of the
table — and scans the frontier inside it, a cross product per recursive step.
It also picks `ix_relations_in` for *both* branches, so the out-edge index is
never used at all. `CROSS JOIN` pins the order and both indexes are used
correctly.

```
JOIN, no stats                    CROSS JOIN, no stats
  SEARCH e USING ix_rel_in (scope=?)  SCAN x
  SCAN x                              SEARCH e USING ix_rel_out (scope=? AND src=?)
```

This reconciles the two reports exactly: **this** benchmark runs `ANALYZE`
after building every index, so it sat in the safe column throughout and the
trap was invisible. The shipped code had no statistics, so it fell straight in.

**The rule: write `CROSS JOIN` in every recursive term.** A recursive CTE has
no statistics for its own frontier, so the planner is guessing; `CROSS JOIN`
removes the guess. `ANALYZE` also fixes it, but statistics are absent on a
fresh database, go stale as the graph grows, and depend on someone remembering
to re-run them — so `ANALYZE` is a nicety and `CROSS JOIN` is the fix. Every
query in §6 and [`MIGRATION.md`](MIGRATION.md) has been corrected. Ordinary
joins over real tables — the FTS5 seed join, the BFS frontier join — are left
alone; they have statistics and do not need pinning.

### 3.2 The headline number was for a query nobody asks

The 0.37 ms figure is the **directed** traversal: follow out-edges only. The
question a person actually asks — *"what is related to this"* — is
**undirected**, needs two recursive terms, costs more, and returns far more.
On the shipped code at 100k edges that is **92 ms p50 / 156 ms p95**, and
**291 ms** from the highest-degree entity.

Both numbers are correct and they are two orders of magnitude apart, which is
worth saying plainly rather than quietly preferring the flattering one. The
conclusion does not move: 92 ms sits inside a 250 ms UI tick, the hard case is
291 ms and still interactive, and neither an extension nor a second engine was
needed to reach them. But **H1's threshold does not survive**: p95 156 ms
against a stated 50 ms. The design claim held; the number did not.

`MEASURED.md` also strengthens **H3** — every 3-hop query hit its 500-row cap,
from ordinary entities as well as hubs — and confirms **H6** independently
(87.47 ms filtered against 92.24 ms unfiltered: bitemporal filtering is again
*faster* than no filtering).
...
Measured at **100,000 edges / 12,500 entities**, SQLite 3.46.1. Each cell is **p50 / p95 ms**; `—` is a query class the design cannot answer; ⏱ marks a class where at least one sample hit its ceiling and is recorded at the ceiling.

| # | design | Q1 1-hop | Q2 3-hop dir | Q3 3-hop undir | Q4 as-of | Q5 path | Q6 hybrid | size | score |
|---|--------|---|---|---|---|---|---|---|---|
| **1** | no index, `UNION ALL` | 74.567 / 83.414 | 79.661 / 82.432 | 3916.231 / 5000.0 ⏱ | — | 231.677 / 251.314 | — | 26.2 MB | **3.416** |
| **2** | no index, `UNION` dedup | 74.038 / 75.592 | 76.367 / 76.533 | 3754.297 / 4867.369 | — | 229.842 / 252.292 | — | 26.2 MB | **3.416** |
| **3** | index on `(src)` | 0.081 / 0.17 | 1.254 / 1.807 | 4048.277 / 4993.669 | — | 273.89 / 348.139 | — | 26.2 MB | **3.416** |
| **4** | covering `(src, dst)` | 0.068 / 0.105 | 0.474 / 0.581 | 2695.343 / 3365.225 | — | 154.782 / 174.462 | — | 26.4 MB | **3.416** |
| **5** | + reverse `(dst, src)`, two recursive terms | 0.048 / 0.09 | 0.387 / 0.486 | 7.208 / 10.784 | — | 83.499 / 105.399 | — | 27.8 MB | **4.166** |
| **6** | mirrored rows in `relations_u` | 0.074 / 0.1 | 0.387 / 0.528 | 7.132 / 10.721 | — | 162.796 / 185.442 | — | 32.5 MB | **3.966** |
| **7** | scope-first covering + temporal pushdown | 0.073 / 0.12 | 0.274 / 0.442 | 5.862 / 8.646 | 0.154 / 0.292 | 63.404 / 74.558 | 25.732 / 28.7 | 32.4 MB | **4.500** |
| **8** | same storage, temporal **post**-filter | 0.056 / 0.093 | 0.267 / 0.429 | 5.319 / 7.55 | 0.74 / 1.219 | 64.538 / 83.873 | 26.435 / 31.301 | 32.4 MB | **4.500** |
| **9** | materialised 2-hop closure | 0.06 / 0.092 | 0.378 / 0.502 | 8.405 / 14.424 | — | 81.694 / 103.262 | — | 44.5 MB | **3.666** |
| **10** | denormalised JSON adjacency | 0.06 / 0.11 | 0.414 / 0.538 | — | — | — | — | 26.2 MB | **3.934** |

What changed at each step, and what it bought:

- **1. no index, `UNION ALL`** — Baseline. A bare heap table, and a recursive CTE with `UNION ALL` and a depth cap but no deduplication. Worst core p95 **5000.0 ms**, rubric **3.416**.
- **2. no index, `UNION` dedup** — Same storage. `UNION` instead of `UNION ALL`, so the recursion deduplicates `(node, depth)` and a cycle terminates. Worst core p95 **4867.369 ms**, rubric **3.416**. No material change in the worst core query.
- **3. index on `(src)`** — Add `relations(src)`. The recursive step can seek instead of scan, but still reads the table for `dst`. Worst core p95 **4993.669 ms**, rubric **3.416**. No material change in the worst core query.
- **4. covering `(src, dst)`** — Widen it to `relations(src, dst)`. The step is now covering: the table is never touched. Worst core p95 **3365.225 ms**, rubric **3.416**. **1.5x faster** than the previous step.
- **5. + reverse `(dst, src)`, two recursive terms** — Add `relations(dst, src)` and rewrite undirected traversal as two recursive terms, one per index, replacing the `OR` join. Worst core p95 **10.784 ms**, rubric **4.166**. **312x faster** than the previous step.
- **6. mirrored rows in `relations_u`** — Materialise every edge twice in `relations_u(a, b)` so undirected traversal is a single recursive term over one index. Worst core p95 **10.721 ms**, rubric **3.966**. No material change in the worst core query.
  - Declared score note: simplicity 3: every edge exists as two rows, so every write and every supersession has to touch both, and 'is this row the edge or its mirror' becomes a thing callers can get wrong.
- **7. scope-first covering + temporal pushdown** — Scope-first covering indexes `(scope, src, valid_to_ms, valid_from_ms, dst)` and the mirror, plus `relations(fact_id)`. Validity is pushed into the recursive step. Worst core p95 **8.646 ms**, rubric **4.500**. Not comparable to the previous step: it answers a different set of query classes.
- **8. same storage, temporal **post**-filter** — Identical storage to I7. The validity predicate is applied *after* the traversal instead of inside it — the regression probe. Worst core p95 **7.55 ms**, rubric **4.500**. No material change in the worst core query.
- **9. materialised 2-hop closure** — Precompute every directed 2-hop pair into `closure2`, and answer 3-hop as closure ⋈ relations. Worst core p95 **14.424 ms**, rubric **3.666**. Not comparable to the previous step: it answers a different set of query classes.
  - Declared score note: simplicity 1 and multiproc 4: a derived table that must be invalidated on every edge write. Under concurrent writers the closure is stale between the write and the rebuild, so a reader can see a neighbour that no longer exists.
- **10. denormalised JSON adjacency** — Denormalise: one JSON array of out-neighbours per entity, expanded with `json_each`. Worst core p95 **0.538 ms**, rubric **3.934**. Not comparable to the previous step: it answers a different set of query classes.
  - Declared score note: simplicity 2 and multiproc 4: an adjacency blob is a read-modify-write on every edge insert — exactly the pattern the store's own research says never to use for contended state.

### Ranking

Sorted by weighted score, not by iteration order. Criteria and weights are fixed in [`RUBRIC.md`](RUBRIC.md): latency 0.25, query power 0.20, build 0.15, one-file 0.10, multi-process 0.10, maintenance 0.10, simplicity 0.10.

| rank | # | design | latency | power | build | file | proc | maint | simple | **total** |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | 7 | scope-first covering + temporal pushdown | 3 | 5.0 | 5 | 5 | 5 | 5 | 5 | **4.500** |
| 2 | 8 | same storage, temporal **post**-filter | 3 | 5.0 | 5 | 5 | 5 | 5 | 5 | **4.500** |
| 3 | 5 | + reverse `(dst, src)`, two recursive terms | 3 | 3.33 | 5 | 5 | 5 | 5 | 5 | **4.166** |
| 4 | 6 | mirrored rows in `relations_u` | 3 | 3.33 | 5 | 5 | 5 | 5 | 3 | **3.966** |
| 5 | 10 | denormalised JSON adjacency | 5 | 1.67 | 5 | 5 | 4 | 5 | 2 | **3.934** |
| 6 | 9 | materialised 2-hop closure | 3 | 3.33 | 5 | 5 | 4 | 5 | 1 | **3.666** |
| 7 | 1 | no index, `UNION ALL` | 0 | 3.33 | 5 | 5 | 5 | 5 | 5 | **3.416** |
| 8 | 2 | no index, `UNION` dedup | 0 | 3.33 | 5 | 5 | 5 | 5 | 5 | **3.416** |
| 9 | 3 | index on `(src)` | 0 | 3.33 | 5 | 5 | 5 | 5 | 5 | **3.416** |
| 10 | 4 | covering `(src, dst)` | 0 | 3.33 | 5 | 5 | 5 | 5 | 5 | **3.416** |

**Winner: iteration 7 — scope-first covering + temporal pushdown**, at 4.500. It is not the fastest cell in every column, and it is not the last iteration run; it wins because it is the only design that answers all six query classes while staying one table plus indexes in one file.

### Breaking the tie between 7 and 8

**Iterations 7 and 8 both score 4.500, and the rubric as fixed cannot separate
them.** That is a defect in the rubric, not a coincidence: its latency
criterion takes the *worst* core p95, which for both designs is Q3 — and Q3 is
identical by construction, since 7 and 8 differ only in where the validity
predicate goes. The 8.646 / 7.550 ms gap between them on Q3 is run-to-run
noise and must not be used to pick a winner.

The tie is broken on **Q4, the only class the two designs actually differ in**:

| | Q4 as-of 3-hop, p50 / p95 |
|---|---|
| **7 — predicate pushed into the recursive step** | **0.154 / 0.292 ms** |
| 8 — same storage, predicate applied after traversal | 0.740 / 1.219 ms |

**4.8x.** Pushdown wins, and the mechanism is the point: an edge that was not
valid at the as-of instant is never *expanded*, so the saving compounds with
depth rather than being paid back at the end. This tie-break rule was added
after seeing the tie, which is worth stating plainly — the honest reading is
that the rubric should have scored per-class latency rather than worst-case,
and a re-run would use that.

### What the loop actually taught

1. **One change was worth more than all the others combined.** Iteration 5 —
   a reverse index plus two recursive terms in place of an `OR` join — took
   the undirected 3-hop from **2,695 ms to 7.2 ms**, a **374x** improvement.
   Everything from iteration 1 to 4 left Q3 between 2.7 and 4.0 *seconds*,
   because `ON (e.src = r.node OR e.dst = r.node)` cannot use an index no
   matter how many you build. If only one line of this report survives, it is
   that one.
2. **`UNION` versus `UNION ALL` is a correctness mechanism, not an
   optimisation.** Iteration 1 → 2 moved Q3 from 3,916 ms to 3,754 ms —
   nothing. The dedup earns its place by making a cyclic graph *terminate*
   without a visited table, and that is why it is in the recommendation; it is
   not where the speed comes from.
3. **Indexing the traversal column bought 920x on the query it serves and
   nothing on the one it does not.** Iteration 3 took Q1 from 74.567 ms to
   0.081 ms, and left Q3 exactly where it was. An index only helps a query
   shaped to use it — which is the same lesson the scope-first index teaches
   in the other direction (below).
4. **Three iterations made things worse, and that is the useful half of the
   log:**
   - **Iteration 6 (mirrored rows)** matched I5 on Q3 (10.721 vs 10.784 ms
     p95) and regressed **shortest path from 83.5 to 162.8 ms — 1.95x** — for
     **+4.7 MB**. The mechanism is instructive: materialising both directions
     tempts you to drop the reverse index, and then every query you did *not*
     rewrite onto the mirror table falls off its index. Denormalisation is not
     local.
   - **Iteration 9 (materialised 2-hop closure)** cost **+12.1 MB** (44.5 vs
     32.4) and bought nothing: Q2 was **no faster than iteration 5** (0.502 vs
     0.486 ms p95) and Q3 got **worse** (14.424 vs 10.784). Precomputation
     cannot beat a traversal that is already index-bound and touching only its
     own neighbourhood — it just adds a structure to invalidate on every write.
   - **Iteration 8 (temporal post-filter)** is the regression this loop was
     built to produce: 4.8x on Q4, above.
5. **Denormalisation lost on power, not on speed.** Iteration 10's JSON
   adjacency answers Q1 and Q2 perfectly well (0.538 ms p95) but **cannot
   answer four of the six classes** — no reverse adjacency, no per-edge
   validity columns, no `fact_id` to seed from FTS5. Its score falls to 3.934
   almost entirely through query power (1.67 of 5).
6. **Two of the largest numbers in this whole report came from schema
   mistakes, not from design trade-offs.** A missing `relations(fact_id)`
   index made the hybrid query **533 ms instead of 6.5 ms** (82x), and a
   `facts` table built with `CREATE TABLE … AS SELECT` — which silently drops
   the `INTEGER PRIMARY KEY` — made it **3,347 ms instead of 3.8 ms** (881x).
   Both were found by reading `EXPLAIN QUERY PLAN` after a number looked
   wrong. Neither is exotic, and either would have shipped unnoticed.
7. **A scope-first covering index is dead to any query that omits scope.**
   The first version of iteration 7 reused the scope-free undirected query and
   a 3-hop went from milliseconds to a full scan per recursion step. This is
   not a flaw — scope is a hard partition Jod applies to every query anyway —
   but it is a rule the design *imposes*, and it is the sharpest edge in the
   recommendation.
...
3. **`cypher()` is a scalar returning a JSON string** —
   ...
   in §6, which is the entire reason to have a graph, cannot be written in it.
...
**Migration `0004_memory_graph` — already applied** in
[`core/src/store.rs`](../../core/src/store.rs), and the DDL there matches what
follows line for line. This is **iteration 7**, the winner of §4. Longer
commentary in [`MIGRATION.md`](MIGRATION.md); the implementation is
re-measured in [`MEASURED.md`](MEASURED.md).

The `CROSS JOIN` correction (rule 1) is also already in `neighbourhood` and
`recall_expanded`. The rules below are therefore a record of *why* the shipped
code looks the way it does, and a checklist for anyone adding a traversal to
it — every one of them is a mistake that measured between 4.8x and 3,515x.

Seven rules come out of this work, and they are as much the deliverable as
the DDL is. Each is followed by the measurement that established it:

1. **`CROSS JOIN`, never `JOIN`, in every recursive term.** Without statistics
   the planner inverts the join order and builds a cross product per step:
   **1,076 ms against 0.32 ms** on the same file (§3.1). First on the list
   because it is the one that silently costs three orders of magnitude. Run
   `ANALYZE` after building the indexes as well, but do not rely on it — a
   fresh database has no statistics.
2. **`UNION`, not `UNION ALL`, in the recursive term** — it deduplicates
   `(node, depth)` so a cycle terminates without a visited table. This is
   correctness, not speed (I1 → I2: no material change).
3. **Two recursive terms for undirected traversal, one per index.** A single
   `ON (e.src = ? OR e.dst = ?)` cannot use an index at all — **312x** on the
   worst core query (I4 → I5).
4. **Push the validity predicate *into* the recursive step**, never after it —
   **4.8x** on the as-of query (I7 vs I8), and the saving compounds with depth.
5. **Every traversal must carry `?scope`.** With `scope` leading the covering
   index, a query that omits it falls back to a full scan per recursion step
   (found while building I7).
6. **Index `relations(fact_id)`** — 533 ms → 6.5 ms on the hybrid query, an
   82x difference from one line of DDL.
7. **Default depth 2, hard cap 3, and cap the result rows.** A relevance
   decision, not a performance one: 4 undirected hops returned 75% of every
   entity in the database, and on the shipped code every 3-hop query saturated
   a 500-row cap.

Two things deliberately **not** in SQL: shortest-path *routes* (a bidirectional
BFS in Rust) and community detection (a periodic job writing
`entity_community`). And two rejected by the loop: a materialised 2-hop closure
(I9 — +12.1 MB, no faster) and a denormalised adjacency blob (I10 — answers two
of six query classes).
...
WITH RECURSIVE reach(node, depth) AS (
  ...
  SELECT e.dst, r.depth + 1
    FROM reach r
    CROSS JOIN relations e ON e.src = r.node AND e.scope = ?3
   WHERE r.depth < ?2
...
WITH RECURSIVE reach(node, depth) AS (
  ...
  UNION
  SELECT e.dst, r.depth + 1
    FROM reach r CROSS JOIN relations e ON e.src = r.node AND e.scope = ?3
   WHERE r.depth < ?2
     AND (e.valid_to_ms IS NULL OR e.valid_to_ms > ?4)
  UNION
  SELECT e.src, r.depth + 1
    FROM reach r CROSS JOIN relations e ON e.dst = r.node AND e.scope = ?3
   WHERE r.depth < ?2
     AND (e.valid_to_ms IS NULL OR e.valid_to_ms > ?4)
)
SELECT en.id, en.name, MIN(reach.depth) AS hops
...
WITH RECURSIVE reach(node, depth) AS (
  ...
  UNION
  SELECT e.dst, r.depth + 1
    FROM reach r CROSS JOIN relations e ON e.src = r.node AND e.scope = ?4
   WHERE r.depth < ?3
  UNION
  SELECT e.src, r.depth + 1
    FROM reach r CROSS JOIN relations e ON e.dst = r.node AND e.scope = ?4
   WHERE r.depth < ?3
)
...
- [`bindings/rust/build.rs`](https://raw.githubusercontent.com/colliery-io/graphqlite/main/bindings/rust/build.rs)
  ...
  — the prebuilt-binary and runtime-extraction mechanism quoted in §5
