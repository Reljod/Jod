...
> **Shipped.** Kept because several task files and one code comment cite it, and
> because the numbered checks below are the names the tests still carry. It is a
> record of what was built, not work to pick up.
>
> It arrived here from the unmerged branch `docs/spec-ceo-and-managers`, where it
> sat as a root `SPEC.md`, and it is unchanged apart from this note and the
> section at the end. Pull request #120 reviewed it claim by claim against main
> and found it executable with four corrections; those corrections and the seven
> questions it left open are answered under
> [What shipped, and what was decided on the way](#what-shipped-and-what-was-decided-on-the-way).
>
> Two things drifted between the review and the build, and are recorded here so
> nobody re-derives them: the migrations landed as `0020`, `0021` and `0022`
> rather than the `0018` the review predicted, because main moved three
> migrations in between; and the line numbers throughout are from August 2026 and
> should be treated as symbol names with a hint attached.
...
## What shipped, and what was decided on the way

Three pull requests, in this order. Each one's checks are the numbered ones
above, and the test names carry those numbers in their doc comments.

| PR | What it covers | Checks |
|---|---|---|
| Change 1 | every session is watched; a stall is marked, not killed | 1–6, 17 |
| Changes 2 and 3a–3d | `works.project_id`, the wider `AgentView`, the manager conversation, `ask_manager`, the two refusals, two preambles | 7–14 |
| Change 3e | project and manager levels in the fleet tree | 15, 16 |

### The four corrections from #120

1. **A manager must not use `pinned = 1`.** Applied. A manager is found through
   `projects.manager_conversation_id` and its `pinned` stays 0, held by
   `creating_a_manager_does_not_disturb_the_main_chat`.
2. **Number the migrations.** They are `0020_a_stalled_session_is_marked`,
   `0021_a_work_knows_its_project` and `0022_a_project_gets_a_manager`. The
   review predicted `0018`; main moved three migrations in between, which is the
   review's own point about not deriving the number by counting.
3. **Three line numbers had drifted.** They have drifted further. Search by
   symbol name.
4. **`open_work` should set the conversation's project.** Applied, in
   `prepare_work`, alongside `works.project_id`.

### The seven open questions

Answered by adopting #120's own recommendations. Any of these can be reversed
without touching the rest.

1. **How a manager's answer gets back to Reljod** — it raises a card, which
   already cascades to main's rail. Told to, in `manager_preamble`.
2. **What stops main doing repository work through `delegate`** — a `cwd` that
   `project_for_path` matches to a catalogued project is refused when the caller
   is main. A repo-less `delegate` still works, so main can still answer "what's
   the weather in Manila" without opening a work.
3. **An ambiguous or unknown project** — main asks, and does not fall back to
   the most recently touched project. Falling back to recency turns a visible
   mistake into an invisible one.
4. **Goals and schedules** — they stay with main. Arming a schedule spends money
   at 2am with nobody watching, and handing that to a per-project manager
   multiplies the things that can arm one by the number of repositories.
   A manager runs at `ToolAccess::Delegate`, which is what enforces it.
5. **One manager per project, or per project per harness** — one per project,
   resumed through `resume_for` the way main is. Splitting it by harness would
   split the memory that is its whole reason to exist.
6. **Retiring a manager** — never. Archiving the project is the deliberate act
   that says you are done, and `should_compact` already applies unchanged.
7. **Splitting the heartbeat work** — split, as its own pull request.

### Two things that are true and are not in the spec above

Both found while checking it, and both recorded by #120:

- **Routing is already deterministic.** `settle_project` runs on the raw
  instruction before the model turn, so by the time main reads anything the
  project is settled. `ask_manager` is wiring, not judgement.
- **`open_work` never touched the catalog at all**, so the child session did not
  inherit the project either. Fixed with the column.

### Still out of scope

The spec's own list stands, and nothing here narrowed it: no automatic killing
of stalled sessions, no backfill of `works.project_id` onto old works, no
managers for `domains/`, and the old JSON router is left alone.

One thing the build added to that list. A project appears in the fleet tree only
when it has a work; the tree is the fleet, not the catalog. A project with a
manager and nothing running does not get a row.
