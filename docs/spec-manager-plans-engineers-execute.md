...
> **Shipped** in `8f9043b` (#228). Kept because the code cites its ids by name —
> `D3.3` in `core/src/leases.rs`, `D4.4` in `core/src/mcp.rs` and
> `core/src/orchestrator.rs`, `D5.2` in `core/src/orchestrator.rs` — and because
> the numbered checks below are the names the tests still carry. It is a record
> of what was built, not work to pick up.
>
> One decision it made has since been **reversed**: the manager was told to hand
> any instruction to any free engineer, whatever the subject. See
> [`SPEC.md`](../SPEC.md) — *an engineer is reused on its own subject only* — and
