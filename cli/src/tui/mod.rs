...
fn apply_slash(app: &mut App, slash: command::Slash) -> Option<Action> {
    ...
    match slash {
        ...
        // which of the three this is — and `bare` is the user having decided for
        // it, asking to cross with nothing carried at all.
...
mod tests {
    ...
    /// The interaction between the two halves of this, which neither could
    /// have been tested for on its own: a thread that would have crossed whole
    /// is still left behind when the user asked for `bare`.
    ///
    /// `bare` exists for a harness that is misbehaving, and "it fits, so I
    /// carried it anyway" would be the crossing refusing the one instruction it
    /// was given.
    #[test]
    fn bare_leaves_a_thread_behind_even_when_it_would_have_fitted() {
        let s = store();
        let (app, thread) = talking_into(&s, HarnessKind::ClaudeCode);
        let id = thread.conversation.clone().unwrap();
        ...
        assert_eq!(
            crossing(Some(&s), &app, &thread, HarnessKind::OpenCode, true),
            Crossing::LeaveBehind { conversation: id }
        );
    }
