...
async fn main() {
    ...
    let harness = match args.next().as_deref() {
        ...
        other => {
            eprintln!("usage: delegate <claude_code|open_code> <prompt> [model]");
            eprintln!("got: {other:?}");
            std::process::exit(2);
        }
    };
...
fn print_event(event: &AgentEvent) {
    match event {
        AgentEvent::Started { session_id, model } => {
            println!("[started]  {} {}", model.as_deref().unwrap_or("-"), session_id.as_deref().unwrap_or("-"));
        }
        ...
        AgentEvent::ToolResult { name, is_error, .. } => {
            println!("[tool ←]   {name}{}", if *is_error { " (error)" } else { "" })
        }
