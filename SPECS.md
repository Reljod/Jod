...
|---|---|---|
| Secrets travel through the spawn contract as **names**, not values | `spawn.json` is written to disk so a person can audit what ran; a value in it would be a second copy of the credential at ordinary permissions — the leak the design exists to prevent, created by the file meant to make it auditable | high |
| One `pending_deliveries` queue for card answers, mail and human nudges | "Is this session ready to be spoken to" is the same question whoever is speaking, and `team::wake_order` already answers it correctly once | high |
| MCP caller identity resolves the server's **process group** against `runs.pgid`, and refuses when a claimed id disagrees | A model cannot argue its way into a different process group, so `from` has no argument anywhere in its derivation. Refusing on disagreement rather than picking a winner, because a mismatch means something is wrong and quietly choosing is how a wrong answer becomes permanent | high |
| D7's inlining branch **deleted** rather than kept | Measured: all three harnesses expand their own commands (Claude Code and AGY from the prompt, OpenCode via `run --command`). A branch kept "just in case" is a branch nobody exercises | high |
| OpenCode's extra roots reach the agent as prose, and Jod does not claim to have granted them | Measured: `--dir` takes exactly one directory and repeating it kills the process before any model call. Overwriting `--dir` with a root would silently move the project the run happens in — a worse failure, because it would look like it worked | high |
| **Known debt:** the A2A ticker batches mail at the bus level rather than through `pending_deliveries`, so two delivery-state vocabularies share four words | The lanes ran concurrently and found each other late. Unification direction agreed: `post` also enqueues, `tick_mail` goes through `plan_injection`, and `claim_wake` is **kept** — it is a rate limit *across ticks*, which `plan_injection` has no equivalent for, since that only declines while a turn is in flight. Blocker: `plan_injection` is keyed on conversation id and explicit team members have none | medium — the plan is sound, the wiring is not yet done |
